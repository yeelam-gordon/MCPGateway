...
After restart, use `list_servers`, one focused `search_tools` query against a chosen alias, `get_tool_schema` for one selected tool, and one harmless read-only `call_tool` if available. Do not load a full backend catalog unnecessarily. Report exactly what was exercised; discovery is not proof that every backend works. For Playwright, verify the exclusive lease rather than claiming isolated browser contexts.

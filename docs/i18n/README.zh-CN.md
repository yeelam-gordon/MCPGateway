...
一个共享 MCP 目录可供多个智能体使用：先在 Copilot 中配置 **10** 个连接，再明确迁移一份受支持且包含 **2** 个新连接的 Claude 配置，两个智能体就都能使用同一组 **12** 个连接；仅安装插件不会自动合并它们。只有同名条目的别名定义完全相同时才会去重，而不能仅凭它们指向同一服务；发现冲突时会停止并等待审查。迁移会先显示预览并创建备份，不支持的原生设置会被拒绝，这也不表示每个原生客户端都已经完成端到端测试；请参阅[迁移指南（英文）](../CLIENTS.md#cross-client-migration)。

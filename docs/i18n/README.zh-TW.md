...
一個共用 MCP 目錄可供多個代理程式使用：先在 Copilot 中設定 **10** 個連線，再明確移轉一份受支援且包含 **2** 個新連線的 Claude 設定，兩個代理程式便都能使用同一組 **12** 個連線；只安裝外掛程式不會自動合併它們。只有同名項目的別名定義完全相同時才會去除重複，而不能只因它們指向同一服務；遇到衝突時會停止並等待檢閱。移轉會先顯示預覽並建立備份，不支援的原生設定會被拒絕，這也不表示每個原生用戶端都已完成端對端測試；請參閱[移轉指南（英文）](../CLIENTS.md#cross-client-migration)。
